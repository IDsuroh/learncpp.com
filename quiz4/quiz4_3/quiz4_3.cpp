...
Have the program output the height of the ball above the ground after
...
Note: Depending on the height of the tower,
the ball may not reach the ground in 5 seconds -- that’s okay.
We’ll improve this program once we’ve covered loops.
Note: The ^ symbol isn’t an exponent in C++.
Implement the formula using multiplication instead of exponentiation.
...
double  calculateHeight(double initialHeight, double seconds) {
    double  distanceFallen = gravity_constant * (seconds * seconds) / 2.0;
    double  currentHeight = initialHeight - distanceFallen;
    return (currentHeight > 0.0) ? currentHeight : 0.0;
}

void    printHeights(double initialHeight) {
