...
Take the program below and modify it so that the ball falls for as many seconds as
needed until it reaches the ground. Update the program to use all covered best practices
...
int main()
{
    ...
    for (int sec = 0; ; sec++)    {
	    const double ballHeight = calculateBallHeight(towerHeight, sec);
	    printBallHeight(ballHeight, sec);
        if (ballHeight == 0.0)
            break ;
    }
    return 0;
